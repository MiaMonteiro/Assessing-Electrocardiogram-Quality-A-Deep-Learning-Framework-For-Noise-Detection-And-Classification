...
def plot_12_leads(signals, fs_original=500, fs_target=360, target_duration=10):
    ...
    for i in range(12):
        ...
        ax = axs[row, col]
        ax.plot(time, signals[i, :], label=f'Lead {i + 1}', color='black', alpha=0.7)
...
# Example usage:
dir_path = r'C:\Users\marci\paper_proj_dataset\ptb_xl_360_split\x_test_all_leads'
filename = '8.npy'
...
signal=np.load(os.path.join(dir_path, filename))
plt.plot(signal[:,0])
plt.show()